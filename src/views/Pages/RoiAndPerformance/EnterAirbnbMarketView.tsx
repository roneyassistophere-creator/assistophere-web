...
import {
  MapPin, Settings, Star, Phone, ArrowRight, CheckCircle2, XCircle,
  BarChart3, Target, TrendingUp, Building2, UserPlus, MessageSquare,
  ChevronRight, Layers, CalendarDays, ListChecks, Megaphone,
  AlertTriangle, ClipboardList, Rocket,
} from 'lucide-react'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="space-y-6">
              ...
              <p className="eam-fade-3 text-lg text-base-content/70 leading-relaxed max-w-lg">
                Assistophere helps UK investors and hosts enter the short term rental market
                with the right setup, structure, and understanding from the start.
              </p>
              ...
              <div className="eam-fade-5 flex items-center gap-3 pt-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-base-content/55 mt-0.5">Trusted by UK hosts entering the STR market</p>
                </div>
              </div>
            </div>
            ...
            <div className="hidden lg:block">
              <div className="relative rounded-3xl border border-base-300 bg-base-200/60 backdrop-blur-sm p-7 shadow-xl">
                ...
                <div className="space-y-3">
                  {[
                    { n: '1', label: 'Market Research', sub: 'Location · Demand · Competition', status: 'done' },
                    { n: '2', label: 'Property Setup', sub: 'Interior · Photography · Standards', status: 'done' },
                    { n: '3', label: 'Listing & Launch', sub: 'Airbnb · VRBO · Booking.com', status: 'done' },
                    { n: '4', label: 'Systems & Pricing', sub: 'Calendar · Dynamic Pricing · Comms', status: 'active' },
                    { n: '5', label: 'Operations & Growth', sub: 'Team · SOPs · Scaling', status: 'pending' },
                  ].map(({ n, label, sub, status }) => (
                    <div key={n} className={`flex items-center gap-4 px-4 py-3 rounded-xl border ${status === 'done' ? 'bg-success/6 border-success/20' : status === 'active' ? 'bg-primary/8 border-primary/25' : 'bg-base-100 border-base-300 opacity-55'}`}>
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${status === 'done' ? 'bg-success text-success-content' : status === 'active' ? 'bg-primary text-primary-content' : 'bg-base-300 text-base-content/40'}`}>
                        {status === 'done' ? '✓' : n}
                      </div>
                      <div>
                        <p className={`text-sm font-semibold ${status === 'active' ? 'text-primary' : ''}`}>{label}</p>
                        <p className="text-xs text-base-content/45">{sub}</p>
                      </div>
                      {status === 'active' && <span className="ml-auto text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full">In Progress</span>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="bg-base-200 border-y border-base-300 py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center space-y-6">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold">Starting an Airbnb Business Isn&apos;t Just Listing a Property</h2>
            <p className="text-base-content/65 text-lg leading-relaxed">
              Most people think the process is: get a property, list it online, start earning.
              The reality involves significantly more — and skipping any part of it leads to
              inconsistent results.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 text-left">
              <div className="bg-base-100 rounded-2xl border border-error/15 p-5">
                <p className="text-xs font-bold uppercase tracking-widest text-error/60 mb-3">What people assume</p>
                {['Get a property', 'List it online', 'Start earning'].map(t => (
                  <div key={t} className="flex items-center gap-2 py-1.5 text-sm text-base-content/60">
                    <XCircle className="w-4 h-4 text-error/50 shrink-0" /> {t}
                  </div>
                ))}
              </div>
              <div className="bg-base-100 rounded-2xl border border-primary/15 p-5">
                <p className="text-xs font-bold uppercase tracking-widest text-primary/60 mb-3">What's actually needed</p>
                {['Proper setup', 'Structured systems', 'Clear strategy'].map(t => (
                  <div key={t} className="flex items-center gap-2 py-1.5 text-sm text-base-content/70">
        ...
        <div className="container max-w-7xl mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { point: 'Choosing the wrong property', detail: 'Location, property type, and local demand all determine performance — a poor choice can mean months of underperformance.' },
                { point: 'No pricing strategy', detail: 'Flat pricing leaves revenue on the table during peak periods and keeps occupancy low in quieter months.' },
                { point: 'Poor listing setup', detail: 'Weak photography, incomplete descriptions, and low review counts all suppress booking visibility and conversion.' },
                { point: 'No operational structure', detail: 'Without systems for guest communication, cleaning, and maintenance, every stay becomes a manual exercise.' },
                { point: 'Unrealistic expectations', detail: 'Expecting strong returns from week one without proper setup, strategy, or time to build reviews leads to disappointment.' },
              ].map(({ point, detail }) => (
      ...
      <section className="bg-base-200 border-y border-base-300 py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold">What You Need to Get Started Properly</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 max-w-6xl mx-auto">
            {[
              { icon: MapPin, title: 'Property Selection', desc: 'Choosing the right location, property type, and market segment based on demand data, not guesswork.' },
              { icon: Megaphone, title: 'Listing Setup', desc: 'Creating listings with optimised content, professional photography, and proper configuration for each platform.' },
              { icon: CalendarDays, title: 'Pricing & Calendar', desc: 'Setting up availability windows, minimum stays, and pricing rules that respond to market demand.' },
              { icon: Settings, title: 'Operational Systems', desc: 'Building workflows for guest communication, cleaning coordination, and property maintenance from day one.' },
              { icon: TrendingUp, title: 'Growth Planning', desc: 'Understanding how your first property sets the foundation for adding more — and what needs to be in place before you scale.' },
            ].map(({ icon: Icon, title, desc }, i) => (
              <div key={title} className="bg-base-100 rounded-2xl border border-base-300 p-5 relative">
      ...
      <section className="py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr_1.5fr] gap-px bg-base-300 rounded-3xl overflow-hidden border border-base-300">
            ...
            {[
              {
                area: 'Market Understanding', icon: BarChart3,
                tasks: ['Identifying viable opportunities in your area', 'Understanding seasonal and local demand patterns', 'Evaluating locations against your goals and budget'],
                outcome: 'You enter the market with accurate data, not assumptions.',
              },
              {
                area: 'Business Setup', icon: ClipboardList,
                tasks: ['Creating a structured operational setup', 'Building workflows before the first booking arrives', 'Preparing your operation to handle guests properly from day one'],
                outcome: 'Your business is ready to operate before it launches.',
              },
              {
                area: 'Listing & Launch', icon: Megaphone,
                tasks: ['Setting up optimised listings across relevant platforms', 'Ensuring photography, content, and configuration are launch-ready', 'Preparing the listing to attract and convert bookings'],
                outcome: 'You launch with a listing that is competitive, not just live.',
              },
              {
                area: 'System Setup', icon: Settings,
                tasks: ['Building a communication flow for guest interactions', 'Configuring pricing and calendar management', 'Setting up operational processes for cleaning and maintenance'],
                outcome: 'Systems are in place so operations do not rely on the host for every decision.',
              },
              {
                area: 'Ongoing Support', icon: TrendingUp,
                tasks: ['Guidance in the weeks after launch', 'Reviewing and improving performance as data arrives', 'Supporting decisions as the business grows'],
      ...
      <section className="bg-base-200 border-y border-base-300 py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { point: 'Initial setup takes planning', detail: 'Getting a property ready for Airbnb properly — photography, listing, pricing, systems — takes weeks, not hours. Rushing this stage costs you later.' },
                { point: 'Results improve over time', detail: 'The first 60–90 days are about building reviews and visibility. Performance typically accelerates meaningfully once that foundation is established.' },
                { point: 'Performance depends on management', detail: 'How well the property is managed — guest communication, pricing, reviews, cleanliness — has a direct impact on occupancy and revenue.' },
                { point: 'Systems matter from the start', detail: 'Building systems after launch is significantly harder than building them before. The structure you put in place early determines how scalable the business becomes.' },
              ].map(({ point, detail }) => (
      ...
      <section className="py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-3">
              {[
                { mistake: 'Rushing into property decisions', why: 'Committing to a property without validating local demand or running the numbers first leads to underperforming assets.' },
                { mistake: 'Copying others blindly', why: 'Basing your strategy on what someone else did in a different market or property type rarely translates. Context matters.' },
                { mistake: 'Ignoring systems', why: 'Trying to manage everything manually works at one property, falls apart at two, and becomes impossible at three.' },
                { mistake: 'Underestimating operations', why: 'Guest communication, cleaning coordination, maintenance, and restocking are significant time commitments without the right systems.' },
                { mistake: 'Expecting quick results', why: 'Properties typically take 60–90 days to build review volume and algorithmic visibility. Starting without that expectation leads to premature decisions.' },
              ].map(({ mistake, why }) => (
      ...
      <section className="bg-base-200 border-y border-base-300 py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { step: '1. Guide your setup step by step', detail: 'We walk through every stage of the entry process — property readiness, listing setup, pricing configuration, and system building.' },
                { step: '2. Build structure from day one', detail: 'We put workflows, communication flows, and operational processes in place before you receive your first booking.' },
                { step: '3. Align everything with your goals', detail: 'Whether your goal is income, portfolio growth, or an eventual exit, the strategy we build is designed around your specific situation.' },
                { step: '4. Prepare your business for scale', detail: 'Everything we set up is designed to hold as your portfolio grows — not just to handle the first property.' },
              ].map(({ step, detail }) => (
      ...
      <section className="py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold">Unstructured Entry vs Strategic Entry</h2>
          </div>
          <div className="hidden md:block overflow-hidden rounded-3xl border border-base-300 max-w-3xl mx-auto">
            <div className="grid grid-cols-3">
              ...
              <div className="bg-success/8 px-5 py-3 text-xs font-bold uppercase tracking-widest text-success/70 text-center">Strategic</div>
            </div>
            {[
      ...
      <section className="bg-base-200 border-y border-base-300 py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
            {[
              { icon: UserPlus, who: 'New STR investors', desc: 'Evaluating the market and wanting to enter with a proper plan rather than figuring it out as they go.' },
              { icon: Building2, who: 'First-time hosts', desc: 'Property owners listing for the first time who want to do it properly from day one.' },
              { icon: MapPin, who: 'Property owners', desc: 'Owners considering converting a property into an STR and needing guidance on setup and viability.' },
              { icon: Rocket, who: 'Operators entering STR', desc: 'Businesses or individuals moving into short term rental from another property model.' },
      ...
      <section className="py-20 lg:py-28">
        <div className="container max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {[
              { initials: 'NP', name: 'Natasha P.', role: 'First-time Host · Leeds', quote: 'I had no idea where to start. Assistophere walked me through every step — property setup, listing, pricing, systems. I launched with confidence rather than guessing.' },
              { initials: 'JO', name: 'James O.', role: 'STR Investor · Birmingham', quote: 'I nearly rushed into the wrong property. The market analysis session helped me understand what would actually perform. My first booking came in the first week.' },
              { initials: 'AC', name: 'Amy C.', role: 'Property Owner · Bristol', quote: 'I had a spare flat and wanted to list it on Airbnb. I thought it would be simple. Assistophere set up everything properly and my results have been far better than I expected.' },
        ...
        <div className="container max-w-7xl mx-auto px-4">
          <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-primary/22 via-base-200 to-secondary/15 border border-base-300 px-8 py-16 lg:px-16 text-center shadow-xl">
            ...
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              <p className="text-base-content/65 text-lg leading-relaxed">
                Let&apos;s build a clear strategy and set your business up the right way.
                We don&apos;t just help you start — we help you enter the market properly.
              </p>
              ...
              <p className="text-xs text-base-content/40 pt-2">
                Enter Airbnb Market UK · How to Start Airbnb UK · STR Business UK · Short Term Rental Investment UK
              </p>
            </div>
          </div>
        </div>
